...
fn initialise_state(code: Vec<char>, input: Vec<u8>) -> State {
    State {
        pos: 0,
        pos_tape: Vec::new(),
        neg_tape: Vec::new(),
...
impl State {
    ...
    fn get_bit(&self) -> bool {
        if self.pos >= 0 {
            self.pos_tape.get(self.pos as usize).map(|b| *b).unwrap_or(false)
        }
        else {
            self.neg_tape.get(-self.pos as usize - 1).map(|b| *b).unwrap_or(false)
        }
    }

    fn get_input_bit(&mut self) -> bool {
        // Read bits in little-endian order
        let bit_value = self.input[self.input_bit / 8] & (self.input_bit as u8 % 8);
        self.input_bit += 1;
        bit_value != 0
    }
...
fn main() {
    let code = "";
    let input = vec![];
    let mut state = initialise_state(code.chars().collect(), input);
    while state.step() {
    }
}
